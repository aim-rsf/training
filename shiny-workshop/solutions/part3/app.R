...
server <- function(input, output) {
  ...
  output$testedPositiveText <- renderTable({
      if (is.null(input$plot_brush)) {
        ...
        brushedPoints(covid_data, input$plot_brush) |>
          summarise(`Start date` = format(min(date), "%d %b %Y"), 
            `End date` = format(max(date), "%d %b %Y"), 
            `Count` = n(), 
            `Min cases` = min(newCasesBySpecimenDate), 
            `Max cases` = max(newCasesBySpecimenDate))
      }
    })
